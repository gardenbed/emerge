	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedError != "" {
				...
				assert.Equal(t, tc.expectedNullable, res.Nullable())
				assert.Equal(t, tc.expectedFirstPos, res.FirstPos())
				assert.Equal(t, tc.expectedLastPos, res.LastPos())
			}
		})
	}
}
